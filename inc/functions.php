...
function createTeacher($link, $teacherName, $teacherImage, $imageType, $imageFile, $subject){
    if($imageType != "jpg" && $imageType != "png"){
        ...
        if(move_uploaded_file($imageFile["tmp_name"], $teacherImage)) {
            ...
            if($stmt->affected_rows > 0){
                besked("success", "Klubben er nu tilføjet");
            }
            $stmt->close();
...
function getTeacher($link){
    ...
    if($stmt->num_rows >= 1){
        ...
        ?>
        <div class="besked rod">Ingen klubber at stemme på</div>
        <?php
    }
...
LIMIT 1';
    ...
    if($stmt->num_rows >= 1) {
        ...
        echo '<div class="besked rod">Ingen klubber at stemme på</div>';
    }
    $stmt->close();
}
...
function deleteSubject($link, $tid){
    ...
    if($stmt->affected_rows > 0){
        ...
        echo "Kunne ikke slette klub fra liga";
    }
    $stmt->close();
}
...
function deleteRating($link, $tid){
    if($GLOBALS["deleteTeacher"] == 1) {
        ...
        if ($rstmt->affected_rows >= 0) {
            ...
            echo "Kunne ikke slette klub fra vurdering";
        }
        $rstmt->close();
    }
}
...
function deleteTeacher($link, $tid){
    if($GLOBALS["deleteTeacher"] == 2){
        ...
        if($tstmt->affected_rows > 0){
            echo "klubben er nu slettet";
        } else {
            echo "Kunne ikke slette klub";
        }
        $tstmt->close();
    }
}